...
# tour always starts at zero and the second value is the lower value of zero's two neighbors
Tour = list[int]
...
def normalize_tour(tour: list[int]):
    i_zero = tour.index(0)
...
def swap_any_not_equal(t0_i: int, t1_i: int, t2_i: int, t3_i: int):
    return t0_i != t1_i and t0_i != t2_i and t0_i != t3_i and t1_i != t2_i and t1_i != t3_i and t2_i != t3_i
...
def is_swap_feasible_by_v(tour: Tour, t0: int, t1: int, t2: int, t3: int):
    return is_swap_feasible(tour, tour.index(t0), tour.index(t1), tour.index(t2), tour.index(t3))


MoveState = tuple[float, int, int, int, int, Tour, set[Edge], set[Edge]]
XResult = Optional[tuple[bool, float, Tour, set[Edge], set[Edge]]]

def select_x(gain: float, t0_i: int, t1_i: int, t2_i: int, t3_i: int, current_tour: Tour, broken_edges: set[Edge], joined_edges: set[Edge], c: Constants) -> XResult:
    _, costs, _ = c
    ...
    if edge_in_edge_list(broken_edge, broken_edges) or edge_in_edge_list(broken_edge, joined_edges):
        return None
...
def select_y(gain: float, t0_i: int, t3_i: int, current_tour: Tour, broken_edges: set[Edge], joined_edges: set[Edge], c: Constants) -> YResult:
    _, costs, best_nbs = c
    t0 = current_tour[t0_i]
    t3 = current_tour[t3_i]
    ...
    for nd in best_nbs[t3]:
        ...
        for nd_nb_i, _ in (tour_succ_i(current_tour, nd_i), tour_pred_i(current_tour, nd_i)):
            ...
            if edge_in_edge_list(joined_edge, joined_edges) or edge_in_edge_list(joined_edge, broken_edges):
                continue
...
def apply_swap(tour: Tour, t0_i: int, t1_i: int, t2_i: int, t3_i: int):
    ...
    split_first = len(tour)-1 == min_i_3 and min_i == 0 and (min_i_1 != 1 or (min_i_1 == 1 and min_i_2 == 2))

    if split_first:
        new_tour = [0] + tour[min_i_2:min_i_3+1] + list(reversed(tour[1:min_i_1+1]))
    else:
        new_tour = tour[:min_i+1] + list(reversed(tour[min_i_1:min_i_2+1])) + tour[min_i_3:]
...
def move_sequence(base_start: MoveState, seen_tours: set[bytes], c: Constants) -> Optional[tuple[Tour, float]]:
    queue: list[MoveState] = [base_start]

    while queue:
        gain, t0_i, t1_i, t2_i, t3_i, current_tour, broken_edges, joined_edges = queue.pop()
        ...
        x_res = select_x(gain, t0_i, t1_i, t2_i, t3_i, current_tour, broken_edges, joined_edges, c)
        ...
        y_res = select_y(gain, mod_t0_i, mod_t3_i, new_tour, broken_edges, joined_edges, c)
        ...
        queue.append((gain, new_t0_i, new_t1_i, new_t2_i, new_t3_i, new_tour, broken_edges, joined_edges))

    return None


def best_move(t0_i: int, t1_i: int, broken_cost: float, current_tour: Tour, seen_tours: set[bytes], constants: Constants) -> Optional[tuple[Tour, float]]:
    t1 = current_tour[t1_i]
    _, costs, best_nbs = constants
    for t2 in best_nbs[t1]:
        t2_i = current_tour.index(t2)
        for _, t3_i in (tour_succ_i(current_tour, t2_i), tour_pred_i(current_tour, t2_i)):
            ...
            maybe_tour_res = move_sequence((gain, t0_i, t1_i, t2_i, t3_i, current_tour, broken_edges, joined_edges), seen_tours, constants)
...
def lin_kernighan2(initial_tour: Tour, costs: Costs):
    n = len(costs)

    last_tour = initial_tour
    new_tour = initial_tour
    best_cost = length_tour(costs, initial_tour)
    ...
    while new_tour is not None:
        # tour_repr = tour_unique(new_tour)
        ...
        while len(untried_tbase) > 0:
            ...
            improve_result = improve_tour(last_tour, tbase, seen_tours, (n, costs, best_nbs))
    ...
    # n = len(initial_tour)
    #
    # better_tour = initial_tour
    #
    # untried_tbase = list(range(n))
    #
    # while True:
    #     maybe_improved = improve_tour(better_tour, (len(initial_tour), costs))
    #     if not maybe_improved:
    #         break
    #
    #     better_tour = maybe_improved
    #
    # return better_tour